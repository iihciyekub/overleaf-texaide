...
`;
  ...
  const readIndex = async (handle) => {
    try {
      ...
      return hydrateCnkiRecordsFromUrlHashes(records, parsed?.urlHashes);
    } catch (_error) {
      return [];
    }
  };
  ...
  const writeIndex = async (handle, records) => {
    ...
    await writable.write(JSON.stringify({
      ...
      urlHashes: buildCnkiUrlHashIndex(records),
  ...
  const synchronizeDirectory = async (handle) => withDirectoryLock(handle, 'index', async () => {
    ...
    for await (const entry of handle.values()) {
      ...
      if (hashes.has(sha256)) {
        duplicateCount += 1;
        const canonical = records.find(record => record.sha256 === sha256);
        if (canonical && cached) mergeCnkiRecordSourceUrls(canonical, cached);
        continue;
      }
      hashes.add(sha256);
      records.push({
  ...
  const downloadCurrentPage = async () => {
    ...
    try {
      ...
      const knownUrls = new Set(records.flatMap(sourceUrlsForCnkiRecord));
        ...
        try {
          if (knownUrls.has(link.url)) {
            ...
            if (knownHashes.has(sha256)) {
              ...
              if (matchingRecord) {
                mergeCnkiRecordSourceUrls(matchingRecord, {
                  sourceUrl: link.url,
