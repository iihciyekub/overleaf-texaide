...
4. Do not change "and" outside OG=(...).`;
    ...
    const refreshWosQueryModelHint = async () => {
        ...
        const allSettings = (await requestStorage("get", "wosAiProviderSettings")) || {};
        const saved = allSettings[provider] || {};
        let model = (saved.model || '').toString().trim();
        if (!model) {
        ...
        wosQueryProviderBadge.textContent = providerNames[provider] || 'OpenAI';
        wosQueryProviderBadge.style.background = isLocal ? '#f1f8f3' : '#eef5fb';
        wosQueryProviderBadge.style.color = isLocal ? '#1f6a43' : '#1a5d93';
        wosQueryProviderBadge.style.borderColor = isLocal ? '#cfe6d7' : '#d2e3f2';
        wosQueryModelIcon.innerHTML = isLocal
            ? '<i class="fa-solid fa-microchip"></i>'
            : '<i class="fa-solid fa-wand-magic-sparkles"></i>';
        wosQueryModelIcon.style.color = isLocal ? '#1f6a43' : '#174b78';
